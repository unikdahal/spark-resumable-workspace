...
Status: draft, not yet built. This document specifies the architecture for a clean-room,
open-source project — provisionally named `spark-resume` (see Appendix A for naming discussion) —
that generalizes a mechanism proven across several proof-of-concept repositories into a real,
pluggable, production-grade library. It intentionally contains no reference to any prior private
design note, prototype name, or internal repository; everything it specifies is re-derived here
from first principles and from the empirical lessons summarized in "What the prototypes taught
us" below.
...
trait ExchangeStore {
  ...
  /** Backend-specific isolation/identity guard (lesson 5, §4): called before ANY reattach
    * decision on this handle, in the current process's backend-identity context. Implementations
    * whose backend has an identity-collision hazard (a resuming process sharing an application
    * identity with a producing process it is NOT resuming from) MUST detect and refuse that
    * collision here, loudly, rather than let it surface as a silent bad read downstream. */
...
case class Anchor(schemaVersion: String, queryId: String, generation: Long,
                   fingerprint: String, handle: ExchangeHandle, createdAtMs: Long)
```

```scala
package org.apache.spark.resume.api

/** Computes a stable identifier for one candidate unit of work (initially: one shuffle-producing
  * stage's physical subtree). Implementations are matched to a specific data-source connector
  * (see lesson 1, §4) -- the SPI is deliberately narrow so a connector author can ship a
  * `SourceFingerprint` implementation alongside their connector without depending on anything
  * beyond the physical-plan node type(s) their connector introduces. */
trait SourceFingerprint {
  /** True if this implementation knows how to fingerprint `node`. The engine tries registered
  ...
  def supports(node: Any): Boolean

  /** Must be a PURE function of `node`'s already-resolved, already-planned state -- no I/O beyond
    * what is needed to read already-cached connector metadata (e.g., a table's already-resolved
    * current snapshot id), and must be safe to call from a hot planning-path context. */
  def fingerprint(node: Any): String
}
```
...
/** One admission check in the chain the `AdmissionEngine` runs before allowing a resume. Rules
  * compose: ALL registered rules must pass for admission to proceed. A rule that cannot decide
  * (insufficient information, backend unavailable) must return `Abstain`, which is treated as a
  * REJECT for admission purposes (fail-closed, per invariant A-2) but is distinguished in
  * observability from an explicit `Reject` so an operator can tell "we don't know" from "we know
  * this must not resume." */
trait AdmissionRule {
  def evaluate(candidate: AdmissionCandidate): AdmissionVerdict
}

case class AdmissionCandidate(fingerprint: String, anchor: Option[Anchor],
...
1. **Phase 0 — SPI + core admission engine**, no real Spark integration yet: prove the rule-chain,
   fencing, and fail-closed semantics against a scripted/mocked `ExchangeStore`/`AnchorStore`.

...
Not a source of current project status. Temporary paths, partially verified hypotheses, build
status that changed minutes later, and AI ownership notes belong here, not beside canonical
architecture docs. Current topology: repository root `README.md`. Current design:
`docs/upstream/` and `docs/architecture/`.
...
**2026-08-25, final disposition: F9 withdrawn.** The rewritten suite ran green on this machine
(`RecoveryKeyDiscriminationSuite` PASS 4/4, `spark-c1-recovery-key.log`): keys discriminate plans
whose differences sit entirely past the truncation cut-off, fingerprints are independent of
`maxToStringFields`, and representative canonical plans produce deterministic distinct digests.
Truncation is real (test 1 pins it) but harmless to the key. There is no Celeborn key-derivation
defect to file; do not reopen without a new reproduction.
...
## Scope change: Claude now owns all three repositories

The codex budget is exhausted, so the ownership split in
`RESUMABLE-SPARK-HANDOFF-AND-ROADMAP.md` §5 no longer applies. Work packages X1-X5 (Spark) come to
me alongside C1-C7. The build constraints in §2 still hold, and so does the integration contract in
§9 - that one is a design agreement, not an ownership artifact, and nothing about it should change
just because one party now holds both sides of it.

### State of the Spark tree as I take it over

Codex's last commit is `ace8a2de502`, pushed as a checkpoint. What it contains, read from the tree
rather than from any report:

- Recovery SPI and connector API, with `RecoveryTaskCommitStore` moved into a new
  `o.a.s.sql.connector.recovery` package.
- A row-level and transaction surface: `SupportsTransactionRecovery`,
  `TransactionRecovery{Info,Result,State}`, `RecoveryDeltaWriter{,Factory}`,
  `SupportsDeltaBatchWriteRecovery`, `SupportsRecoveryTaskMetrics` and its descriptor/schema types.
- `dev/create-recovery-api-artifact.sh` and `dev/recovery-api-public-classes.txt`, the machinery X1
  step 8 calls for.
- Scheduler-side changes in `DAGScheduler`, `MapOutputTracker`, `MapStatus`, `KryoSerializer`, and a
  new `RecoveredShuffleStatusBenchmark`.

Known-failing when I last ran it, all of it my own scaffolding rather than codex's implementation:

| Suite | State | Note |
|---|---|---|
| `BatchWriteRecoverySuite` | 4 of 5 failing | new suite, fixtures never debugged past the `local[2, 4]` fix |
| `RecoveryKeyDiscriminationSuite` | 3 of 3 failing | the F9 probe; the finding itself is still unproven |
| `RecoveryTaskCommitCompatibilitySuite` | passing | golden fixture recorded and verified |
| `scalastyle:check` | passing | after the eleven violations I fixed |

### Order I will work X1 in

1. Make my own two suites tell the truth - debug them until they either pass or produce a defect
   worth reporting. A red suite nobody trusts is worse than no suite.
2. Run the focused Spark recovery set: envelope, compatibility, extensions, scheduler, map output.
3. Settle F9 with evidence, one way or the other.
4. Only then freeze the API and produce the jar, because freezing a contract whose tests do not run
   is how the Celeborn and Iceberg gates ended up broken in the first place.
